...
def learn(network,samples, epochs=100000000, lrate=.25, momentum=0.1):
    MSE = 1
    it = 0
    stop = 0.001
    # Fase de Treinamento
    ...
    for i in range(samples.size):
        ...
        if(out < 1.5*samples['output'][i] and out > 0.5*samples['output'][i] ):
            acertos = acertos + 1
    print('----------------------------------------')
...
def main():
    ...
    for i in range(n_samp):
        ...
        for j in range(1, n_atr-1):
            ...
            #print(DMAP[i][j])
            DINP[i][j-1] = DMAP[i][j]
        DOUT[i] = remap(DMAP[i][10], 1, 7, 0, 1)
        print(DMAP[i])
       	print(DINP[i])
        print(DOUT[i])        
        print('------')
    ...
    network = MLP(9, 12,  1)
    samples = numpy.zeros(n_tr, dtype=[('input',  float, 9), ('output', float, 1)])
    for i in range(n_tr):
